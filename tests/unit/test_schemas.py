...
class TestSemanticVersion:
    ...
    def test_init(self):
        ...
        version = SemanticVersion(major=1, minor=2, patch=3, source=self.test_path)
    ...
    def test_parse_valid_version(self):
        ...
        version_str = "1.2.3"
        version = SemanticVersion.parse_version(version_str, self.test_path)
        assert version.major == 1
        assert version.minor == 2
        assert version.patch == 3
        assert version.source == self.test_path
    ...
    def test_parse_invalid_version_with_error_raised(self):
        ...
        with pytest.raises(ValueError, match="Invalid version format: 1.2.3.4"):
            SemanticVersion.parse_version("1.2.3.4", self.test_path, raise_error=True)

    def test_comparison_major_version(self):
        """Test comparison based on major version."""
        v1 = SemanticVersion(major=1, minor=0, patch=0, source=self.test_path)
        v2 = SemanticVersion(major=2, minor=0, patch=0, source=self.test_path)
        assert v1 < v2
        assert not (v2 < v1)

    def test_comparison_minor_version(self):
        """Test comparison based on minor version."""
        v1 = SemanticVersion(major=1, minor=1, patch=0, source=self.test_path)
        v2 = SemanticVersion(major=1, minor=2, patch=0, source=self.test_path)
        assert v1 < v2
        assert not (v2 < v1)

    def test_comparison_patch_version(self):
        """Test comparison based on patch version."""
        v1 = SemanticVersion(major=1, minor=1, patch=1, source=self.test_path)
        v2 = SemanticVersion(major=1, minor=1, patch=2, source=self.test_path)
        assert v1 < v2
        assert not (v2 < v1)
    ...
    def test_comparison_equal_versions(self):
        ...
        v1 = SemanticVersion(major=1, minor=1, patch=1, source=self.test_path)
        v2 = SemanticVersion(major=1, minor=1, patch=1, source=self.test_path)
        assert not (v1 < v2)
        assert not (v2 < v1)
