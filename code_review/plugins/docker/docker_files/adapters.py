...
def content_to_python_adapter(content: str) -> DockerImageSchema | None:
    ...
    python_patterns = [re.compile(r"ARG\s.+=(?P<version>\d\.\d+[\.\d]*)\-?(?P<os>[a-z\-]+)"),
                      re.compile(r"python:(?P<version>\d\.\d+[\.\d]*)\-?(?P<os>[a-z\-]+)")]
...
def content_to_node_adapter(content: str) -> DockerImageSchema | None:
    ...
    for node_pattern in node_patterns:
        ...
        if match:
            version = match.group(1)
            return DockerImageSchema(name="node", version=version, operating_system="unknown")
    return None
